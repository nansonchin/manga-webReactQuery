...
vi.mock(
  "../../features/readerNavigation/hooks/useChapterNavigation",
  () => ({
...
vi.mock(
  "../../features/reader/hooks/useLongStripPageTracking",
  () => ({
...
vi.mock(
  "../../features/reader/hooks/useLongStripNavigation",
  () => ({
...
vi.mock(
  "../../features/reader/components/ReaderImage/ReaderImage",
  () => ({
    default: ({
      src,
      alt,
...
function renderReaderPage() {
  return render(
    <MemoryRouter
      initialEntries={["/manga/manga-1/chapter/chapter-1"]}
    >
      <Routes>
        <Route
...
describe("ReaderPage integration", () => {
  ...
  it("renders the Reader with the first page initially", () => {
    renderReaderPage();

    expect(
      screen.getByText("Current Page : 1"),
    ).toBeInTheDocument();

    expect(
      screen.getByAltText("Page 1"),
    ).toBeInTheDocument();

    expect(
      screen.getByAltText("Page 2"),
    ).toBeInTheDocument();

    expect(
      screen.getByAltText("Page 3"),
    ).toBeInTheDocument();
  });


  it("moves to the next page when the user taps the right half", async () => {
    renderReaderPage()
    ...
    expect(
      screen.getByText("Current Page : 2"),
    ).toBeInTheDocument();
  });

  it("moves to the previous page when the user taps the left half", async () => {
    renderReaderPage()
  ...
  it("does not move past the last page", async () => {
    ...
    expect(
      screen.getByText("Current Page : 3"),
    ).toBeInTheDocument();
  ...
  it("updates ReaderProgress when the current page changes", async () => {
    renderReaderPage()
    ...
    expect(
      screen.getByLabelText("Page 1 of 3"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByLabelText("Page 2 of 3"),
    ).toBeInTheDocument();
  });


  it("moves to the next page with ArrowRight", async () => {
    const user = userEvent.setup()
    renderReaderPage()
  ...
  it("moves to the previous page with ArrowLeft", async () => {
    const user = userEvent.setup()
 ...
 await user.keyboard("{ArrowLeft}")
  ...
  it("enables chapter navigation buttons when chapters exist", () => {
  ...
  it("calls the chapter navigation callback when Next Chapter is clicked", async () => {
    const user = userEvent.setup();
  ...
  it("renders loading state while pages are loading", () => {
    ...
    expect(
      screen.getByText("Loading pages ..."),
    ).toBeInTheDocument();
  });
  ...
  it("connects the current page to useReaderPreload", () => {
    renderReaderPage();

    expect(
      mocks.useReaderPreload,
  ...
  it("enables long strip page tracking in long strip mode", () => {
    renderReaderPage();

    expect(
      mocks.useLongStripPageTracking,
    ).toHaveBeenCalledWith({
      enabled: true,
  ...
  it("connects current page and total pages to long strip navigation", () => {
    renderReaderPage();

    expect(
      mocks.useLongStripNavigation,
    ).toHaveBeenCalledWith({
      currentPage: 0,
      totalPages: 3,
    });
  ...
  it("renders an invalid URL message when mangaId is missing", () => {
    ...
    expect(
      screen.getByText("Invalid Reader Url"),
    ).toBeInTheDocument();
  });
});
